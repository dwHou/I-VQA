...
img_path = "./00151001.png"
im = load_image(img_path)
gray_image = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)
# cv2.COLOR_RGB2GRAY

# ndarray 2 opencv
# img = cv2.merge([nd[:,:,2],nd[:,:,1],nd[:,:,0]])  手动RGB2BGR
# img = cv2.merge([nd])
# img = cv2.merge([nd[i,:,:,:]]) or cv2.merge([nd[i]])
# 如果输入是多帧N:H:W:C
...
feats = np.asarray([])
brisque_features1 = np.concatenate((brisque_features, downscale_brisque_features))
feats = np.concatenate((feats, brisque_features1))
# brisque_features2 = np.concatenate((brisque_features, downscale_brisque_features))
# feats = np.concatenate((feats, brisque_features2))
...
# list = []
# list.append(brisque_features1)
# list.append(brisque_features2)
# print(list)
...
# print(type(brisque_features))
# print(brisque_features.shape)
# brisque_features_tensor = torch.from_numpy(brisque_features)
# print(brisque_features_tensor)
# print(brisque_features_tensor.shape)
